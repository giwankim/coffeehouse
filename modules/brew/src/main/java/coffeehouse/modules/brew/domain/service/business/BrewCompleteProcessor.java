...
public class BrewCompleteProcessor implements BrewComplete {
  ...
  public BrewCompleteProcessor(
      BrewNotifier brewNotifier, OrderSheetRepository orderSheetRepository) {
    this.orderSheetRepository =
        Objects.requireNonNull(orderSheetRepository, "OrderSheetRepository must not be null");
  ...
  public void complete(OrderId orderId) {
    ...
    // ----------------------------------------------------------
    var orderSheet =
        orderSheetRepository.findByOrderId(orderId).orElseThrow(OrderSheetNotFoundException::new);
    orderSheet.process();
