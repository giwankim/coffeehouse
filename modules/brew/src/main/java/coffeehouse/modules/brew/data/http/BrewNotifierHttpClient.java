...
public class BrewNotifierHttpClient implements BrewNotifier {
  ...
  public void notifyUser(OrderId orderId) {
    ...
    var request =
        RequestEntity.post(brewCompletedNotificationUserUri)
            .body(new NotifyBrewCompleteRequest(orderId));
  ...
  public void notifyOrder(OrderId orderId) {
    ...
    var request =
        RequestEntity.post(brewCompletedNotificationOrderUri)
            .body(new NotifyBrewCompleteRequest(orderId));
    restTemplate.exchange(request, Void.class);
  }
